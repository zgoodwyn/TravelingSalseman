...
/*	for(int index = 0; index < num_Cities; index++)
	...
	string* least_Path_Array = new string[num_Threads];
...
#pragma omp master
		{
			cout << "Initialized Threads: " << omp_get_num_threads() << endl;
		}
...
/*#pragma omp critical
			{
				...
				outfile << index << ": " << possiblePath.str() << ": " << totSum << endl;
	...
	for(int index = 0; index < num_Threads; index++)
	{
		if(least_Cost_Array[index] < temp_Solution)
		{
			...
			final_Path = least_Path_Array[index];
		}
	}
